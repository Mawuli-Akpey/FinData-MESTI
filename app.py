...
start_time = st.sidebar.text_input("Start Time (HH:MM:SS:SS)", "00:00:00:00")
end_date = st.sidebar.date_input("End Date", pd.to_datetime("2024-07-22"))
end_time = st.sidebar.text_input("End Time (HH:MM:SS:SS)", "23:59:59:00")
...
try:
    ...
    if start_datetime > end_datetime:
        ...
        total_males = filtered_df['Participant Gender'].str.contains('Male', na=False).sum()
        total_females = filtered_df['Participant Gender'].str.contains('Female', na=False).sum()
        ...
        submission_counts = filtered_df['_submitted_by'].value_counts().reset_index()
        ...
        average_time_per_enumerator = filtered_df.groupby('_submitted_by')['form_complete_time'].mean().reset_index()
        ...
        fig = px.histogram(filtered_df, x='Participant Age in years',
        ...
        fig = px.histogram(filtered_df, x='Participant Age in years', color='Participant Gender',
        ...
        platform_registration_counts = filtered_df['Are you registered on any Agritech e-commerce platform?'].value_counts().reset_index()
        ...
        grouped_df = filtered_df.groupby(['_submitted_by', 'Are you registered on any Agritech e-commerce platform?'])['form_complete_time'].mean().reset_index()
        ...
        crop_counts = filtered_df['<span style="display:none">row-Crop Type</span>'].value_counts().reset_index()
        ...
        services_counts = extract_financial_services(filtered_df, financial_services_column)
        ...
        income_counts = filtered_df[income_column].value_counts().reset_index()
        ...
        df_geo = filtered_df[geo_columns].dropna(subset=['_start-geopoint_latitude', '_start-geopoint_longitude'])
